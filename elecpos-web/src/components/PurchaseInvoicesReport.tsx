...
export default function PurchaseInvoicesReport({
  ...
  const summary = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (!ex) {
        map.set(r.inv_id, { inv_id: r.inv_id, inv_date: r.inv_date, subtotal: sub, tax, total: tot, lines: 1 });
      } else {
        ex.subtotal += sub;
        ex.tax      += tax;
        ex.total    += tot;
  ...
  function exportExcel() {
    if (showDetails) {
      ...
      saveXlsx(data, "purchases_details");
    } else {
      ...
      saveXlsx(data, "purchases_summary");
    }
  }

  function saveXlsx(data: any[], sheetName: string) {
  ...
  function exportPDF() {
    ...
    const title = `Purchase Invoices (${meta.from} → ${meta.to}) ${showDetails ? "- Details" : "- Summary"}`;
    ...
    if (showDetails) {
      autoTable(doc, {
        startY: 22,
        head: [["Inv ID", "Date", "SKU", "Name", "Qty", "Unit Cost", "Tax", "Line Total"]],
        body: rows.map((r) => [
          r.inv_id,
          r.inv_date,
          r.sku,
  ...
  function fileName(ext: string) {
    return `purchases_${meta.from}_${meta.to}_${showDetails ? "details" : "summary"}.${ext}`;
  }
    ...
    <div className="rounded-2xl border p-3 bg-white overflow-auto space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          <button className="px-3 py-2 border rounded-2xl" onClick={exportExcel}>تصدير Excel</button>
          <button className="px-3 py-2 border rounded-2xl" onClick={exportPDF}>تصدير PDF</button>
        </div>
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="px-3 py-2 text-right">كلفة مفرد</th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((r, i) => (
                <tr key={i} className="border-t">
                  ...
                  <td className="px-3 py-2 text-right">{Number(r.unit_cost || 0).toLocaleString()}</td>
